import * as Papa from "../../deps/papaparse.ts";

import * as assert from "../assert.ts";
import parseCharacter, { Template } from "./character.ts";
import parseGamebase from "./gamebase.ts";
import parseItem from "./item.ts";
import parseVarSize from "./varsize.ts";
...
export default function parseCSV(content: Map<string, string>): Csv {
  function getTable(name: string) {
    ...
    const filtered = stripped.map((line) => line.trim()).filter((line) =>
      line.length > 0
    );
  ...
  const varSize = parseVarSize(
    "VARIABLESIZE.CSV",
    getTable("VARIABLESIZE.CSV"),
  );
  ...
  for (const name of content.keys()) {
    if (name.startsWith("CHARA") && name.endsWith(".CSV")) {
      const template = parseCharacter(name, getTable(name), {
        base: new Map(
          [...base.entries()].map(([index, value]) => [value, index]),
        ),
        mark: new Map(
          [...mark.entries()].map(([index, value]) => [value, index]),
        ),
        exp: new Map(
          [...exp.entries()].map(([index, value]) => [value, index]),
        ),
        abl: new Map(
          [...abl.entries()].map(([index, value]) => [value, index]),
        ),
        talent: new Map(
          [...talent.entries()].map(([index, value]) => [value, index]),
        ),
        cflag: new Map(
          [...cflag.entries()].map(([index, value]) => [value, index]),
        ),
        equip: new Map(
          [...equip.entries()].map(([index, value]) => [value, index]),
        ),
        juel: new Map(
          [...palam.entries()].map(([index, value]) => [value, index]),
        ),
        cstr: new Map(
          [...cstr.entries()].map(([index, value]) => [value, index]),
        ),
      });
