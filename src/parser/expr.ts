...
import Binary from "../statement/expr/binary.ts";
import Const from "../statement/expr/const.ts";
import Form from "../statement/expr/form.ts";
import InlineCall from "../statement/expr/inline-call.ts";
import Ternary from "../statement/expr/ternary.ts";
import Unary from "../statement/expr/unary.ts";
import UnaryOp from "../statement/expr/unary-op.ts";
  ...
  Binary: (r) => {
    ...
    for (const op of operators) {
      result = P.seqMap(
        ...
        (first, rest) =>
          rest.reduce((acc, val) => new Binary(val[0], acc, val[1]), first),
      );
    }
    ...
    P.alt(
      U.wrap(
        ...
        P.seqMap(
          ...
          U.optional(
            P.string("#").trim(C.WS0).then(U.optional(P.lazy(() => formEnd))),
          ),
          (expr, left, right) =>
            new Ternary(expr, left ?? new Const(""), right ?? new Const("")),
        ),
      ),
      P.seqMap(
        r.Binary,
...
function formParser(exclude: string, withTernary: boolean): P.Parser<Form> {
  ...
  chunkParser.push(U.wrap(
    "{",
    "}",
    P.seqMap(
      language.Expr.trim(C.WS0),
      P.string(",").trim(C.WS0).then(U.optional(language.Expr)).fallback(
        undefined,
      ),
      P.string(",").trim(C.WS0).then(U.optional(U.alt("LEFT", "RIGHT")))
        .fallback(undefined),
  ...
  chunkParser.push(
    C.charSeq("{", "%", "\\@", ...exclude).map((value) => ({ value })),
  );
