...
export function preprocess(lines: Slice[], macros: Set<string>): Slice[] {
	const fn: Array<(prev: Slice[]) => Slice[]> = [
		...
		(prev) =>
			prev.map((line) =>
				new Slice("", line.line, line.content.replace(/;.*$/, ""))
			),
		// Trim whitespaces
		(prev) =>
			prev.map((line) => new Slice("", line.line, line.content.trim())),
		// Remove empty lines
		...
		(prev) => {
			...
			while (index < prev.length) {
				...
				if (line.content === "[SKIPSTART]") {
					index += prev.slice(index).findIndex((l) =>
						l.content === "[SKIPEND]"
					) + 1;
		...
		(prev) => {
			...
			while (index < prev.length) {
				...
				if (line.content === "{") {
					const endIndex = index +
						prev.slice(index).findIndex((l) => l.content === "}");
