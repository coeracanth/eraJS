	...
	public async build(vm: VM): Promise<Value<any>> {
		if (
			...
			const value = await Promise.all(
				this.value.map((v) => v.reduce(vm)),
			);
			assert.bigintArray(
				value,
				"Default value for 1D #DIM must be a number array",
			);
			...
			assert.strArray(
				value,
				"Default value for 1D #DIMS must be a string array",
			);
			return new Str1DValue(this.name, [value.length]).reset(value);
		} else if (
			this.size.length === 0 && this.type === "number" && !this.isChar()
		) {
			return new Int0DValue(this.name);
		} else if (
			this.size.length === 0 && this.type === "string" && !this.isChar()
		) {
			return new Str0DValue(this.name);
		} else if (
			this.size.length === 1 && this.type === "number" && !this.isChar()
		) {
			...
			this.size.length === 1 && this.type === "string" && !this.isChar()
		) {
			const size = await this.size[0].reduce(vm);
			assert.bigint(size, "Size of an array must be an integer");
			return new Str1DValue(this.name, [Number(size)]);
		} else if (
			this.size.length === 2 && this.type === "number" && !this.isChar()
		) {
			...
			this.size.length === 3 && this.type === "number" && !this.isChar()
		) {
			const size0 = await this.size[0].reduce(vm);
			assert.bigint(size0, "Size of an array must be an integer");
			const size1 = await this.size[1].reduce(vm);
