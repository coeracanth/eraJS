...
import { Template } from "./csv/character.ts";
import EraJSError from "./error.ts";
...
import CharaNumValue from "./value/special/charanum.ts";
import LineCountValue from "./value/special/linecount.ts";
import RandValue from "./value/special/rand.ts";
import valueList from "./value-list.ts";
...
export default class VM {
  ...
  public constructor(code: VM["code"]) {
    ...
    this.globalMap.set(
      "GLOBAL",
      new Int1DValue("GLOBAL", code.csv.varSize.get("GLOBAL")),
    );
    this.globalMap.set(
      "GLOBALS",
      new Str1DValue("GLOBALS", code.csv.varSize.get("GLOBALS")),
    );
  }
    ...
    for (const fn of fnList) {
      ...
      for (const property of fn.property) {
        if (property instanceof Dim && !property.isDynamic()) {
          this.staticMap.get(fn.name)!.set(
            property.name,
            await property.build(this),
          );
        } else if (
          property instanceof LocalSize || property instanceof LocalSSize
        ) {
  ...
  public async pushContext(fn: Fn): Promise<Context> {
    ...
    context.dynamicMap.set(
      "ARG",
      new Int1DValue("ARG", this.code.csv.varSize.get("ARG")),
    );
    context.dynamicMap.set(
      "ARGS",
      new Str1DValue("ARGS", this.code.csv.varSize.get("ARGS")),
    );
    for (const property of fn.property) {
      if (property instanceof Dim && property.isDynamic()) {
    ...
    while (true) {
      ...
      switch (result?.type) {
        case "begin":
          ...
          continue;
        case "goto":
          throw E.notFound("Label", result.label);
        case "break":
          return null;
        case "continue":
          return null;
